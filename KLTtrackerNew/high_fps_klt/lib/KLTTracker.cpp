...
trackers::KLTTracker::KLTTracker(SensorData& SF_, double imageScaling_ = 1, double NoFrames_ = 0, bool visualize_=false)
{
	SF = SF_;
	IMU_rate = SF.sampling_rate;
	imageScaling = imageScaling_;
	NoFrames = NoFrames_;
	medianThresh = 5; //default: 5	max 7- 
	min_pt_d = 5; //default: 5 (icke 3)
	epipolarThresh = 1; //default: 1, max 3 
    max_iter = 3; //default: 3
    pyr_lvl = 3;//default: 3
	qualityLevel = 0.01; //default: 0.01
    win_size = cv::Size(3,3); //default: cv::Size(3, 3)
    target_npts = 1100; //default: 1100
    frame_num = 0;
	keyframe_num = 0;
...
void trackers::KLTTracker::writeToFile(string fileName)
{	
	...
	std::vector<cv::Mat> position_vec = SF.getPositions();
	cv::Mat current_pos;

	Point3d position = Point3d(0, 0, 0);

	int N = timestamps.size(), IMU_index;
	double timestamp;
	...
	for (int i = 0; i < N; i++)//
	{
		...
		// Get timestamp for i:th keyframe
		timestamp = timestamps[i];
		// Find position for that given timestamp
		IMU_index = (int)(timestamp*IMU_rate);
		// Get current pos from IMU
		current_pos = position_vec[IMU_index];

		// Lets ignore movement in height, anyhow induced by noise or bias! 
		position.x = current_pos.at<double>(0);
		position.y = current_pos.at<double>(1);
		position.z = current_pos.at<double>(2); //0; 

		textPath = (saveName.c_str());
		pFile = fopen(textPath, "a");  //opens and writes to file
		if (pFile != NULL)
		{
			entry = std::to_string(position.x) + " " + std::to_string(position.y) + " " + std::to_string(position.z) + "\n";
...
void trackers::KLTTracker::computeScale()
{//Get mean scale between poses (KF) from IMU readings
	std::vector<cv::Mat> position_vec = SF.getPositions();
	cv::Mat current_pos;
	Point3d prev_position = Point3d(0, 0, 0);
	Point3d position = Point3d(0, 0, 0);
	...
	IMU_index = (int)(timestamp*IMU_rate);
	prev_position.x = position_vec[IMU_index].at<double>(0);
	prev_position.y = position_vec[IMU_index].at<double>(1);
	prev_position.z = position_vec[IMU_index].at<double>(2);

	//cout << "Number of datapoints to estimate mean distance between poses:" << N << endl;
	for (int i = 1; i < N; i++)
	{
		// Get timestamp for i:th keyframe
		timestamp = timestamps[i];
		...
		norm_vec.push_back(cv::norm(position - prev_position));

		prev_position = position;
	}
	IMU_scale = 0;
	// Add up all lengths and divide by the total
	for (double n : norm_vec)
		IMU_scale += n;
	IMU_scale = IMU_scale / N;
	// Write to file
	saveScale();
	writeToFile("IMUposes.txt");
	//return IMU_scale;
}
double trackers::KLTTracker::getScale()
{
	return IMU_scale;
}
...
void trackers::KLTTracker::process()
{		
	//Track runtime
	...
	cv::Mat greyMat, disp;
	//Open the capture for video file
	...
	else
	{
		...
		{
			//cap.set(CV_CAP_PROP_POS_FRAMES, NoFrames - 1000); //Set index to last frame
			...
			{				
				...
				{// If true we have found a keyframe, extract keypoints for this frame and update key_point coords etc.
					...
					keyPoints();
					timestamps.push_back(timestamp);
					//cout << "RAW timestamp: " << cap.get(CV_CAP_PROP_POS_MSEC) << endl;
					...
					geometricConstraint();		
					//cout << IDs[keyframe_num].size() << endl;
				}
			}
			// If video 360* around the object; add for last frame to connect with first
			// trackLastToFirst();
		}	
		cout << "\nFound " << keyframe_num + 1 << " keyframes with runtime: " << (double)(clock() - tStart) / CLOCKS_PER_SEC << "s."<<endl;
		//printf("%.2fs", (double)(clock() - tStart) / CLOCKS_PER_SEC);
		computeScale();
	}
}
// Local function 
...
{	// Save data at the folder of the video
	...
	*/
	...
	uint numMatches = 0;
	uint numViews = key_frames.size();
	cout << "\n";
	for (int i = 1; i < numViews; i++)
	{		
		progress = int((100.0*(i) / (NoFrames - 1)));
		std::cout << "Writing matches.e/f files: " << progress << " %\r";
		if (pFile != NULL && textFile != NULL)
		{
			for (int k = i; k < i+3; k++)
			{			
			matches = getMatches(IDs[i - 1], IDs[k]);
			numMatches = matches.size();
					
				// Write view IDs of pair of views
				entry = std::to_string(i - 1) + " " + std::to_string(k) + "\n";
				textPath = entry.c_str();
				fputs(textPath, pFile);
				fputs(textPath, textFile);
				// Write number of points matching in the 2 views
				entry = std::to_string(numMatches) + "\n";
				textPath = entry.c_str();
				fputs(textPath, pFile);
				fputs(textPath, textFile);
		...
		{
			cout << "Couldnt write to file (matches.e/f)." << endl;
		}
	}	
	fclose(pFile);
	fclose(textFile);
...
void trackers::KLTTracker::saveScale()
{
	string saveName, entry;
	const char * path(videoPath.c_str());
	const char * textPath;
	...
	_splitpath_s(path, drive, dir, fname, ext);
	saveName = drive + string(dir) + "scale.txt";
	// Convert to const char *
	textPath = (saveName.c_str());
	remove(textPath);
	cout << "Saved IMU-to-camera scale in scale.txt: " << IMU_scale << endl;
	textPath = (saveName.c_str());
	pFile = fopen(textPath, "a");  //opens and writes to file
	if (pFile != NULL)
	{
		entry = std::to_string(IMU_scale);
		textPath = entry.c_str();
		fputs(textPath, pFile);
		fclose(pFile);
	}// close file 			
...
void trackers::KLTTracker::geometricConstraint()
{
	...
	double probability = 0.99; //0.99
	mask.clear();
	//Find F-matrix and save mask to remove points 
	F = findFundamentalMat(prev_points, points, method, epipolarThresh, probability, mask);
...
{//Return true for keyframes; depending on median distance moved for each all tracks
	...
	if (medianDisp > medianThresh || NoPoints < 250)
	{	// If this is achieved we have found a keyframe	and need to generate more points
		return true;
	}
	else
...
{ // Each keyFrame has key_points which should be tracked all the way to the next keyFrame, with known IDs of the points.
	...
	cv::goodFeaturesToTrack(current_frame.getGray(), new_pts, target_npts, qualityLevel, min_pt_d);
